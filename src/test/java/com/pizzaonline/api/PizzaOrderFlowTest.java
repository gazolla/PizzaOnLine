...
public class PizzaOrderFlowTest {
	...
	public void testPizzaOrderFlow() throws Exception {
		...
		MvcResult orderResult = mockMvc.perform(get("/api/orders/" + createdOrder.getId())).andExpect(status().isOk())
				.andReturn();
		...
		updatedOrder = updateOrderStatus(createdOrder.getId(), "READY_FOR_DELIVERY");
		assertEquals("READY_FOR_DELIVERY", updatedOrder.getStatus());
		System.out.println("Status do pedido atualizado para: " + updatedOrder.getStatus());

		updatedOrder = updateOrderStatus(createdOrder.getId(), "OUT_FOR_DELIVERY");
		assertEquals("OUT_FOR_DELIVERY", updatedOrder.getStatus());
		System.out.println("Status do pedido atualizado para: " + updatedOrder.getStatus());

		updatedOrder = updateOrderStatus(createdOrder.getId(), "DELIVERED");
	...
	private void updateOrderWithPayment(Long orderId, Long paymentId) throws Exception {
		MvcResult orderResult = mockMvc.perform(get("/api/orders/" + orderId)).andExpect(status().isOk()).andReturn();
		String orderJsonResponse = orderResult.getResponse().getContentAsString();
		ObjectMapper objectMapper = new ObjectMapper();
		Order order = objectMapper.readValue(orderJsonResponse, Order.class);

		MvcResult paymentResult = mockMvc.perform(get("/api/payments/" + paymentId)).andExpect(status().isOk())
				.andReturn();
		...
		mockMvc.perform(
				put("/api/orders").contentType("application/json").content(objectMapper.writeValueAsString(order)))
				.andExpect(status().isOk());
	}

	private Order updateOrderStatus(Long orderId, String status) throws Exception {
		MvcResult result = mockMvc.perform(get("/api/orders/" + orderId)).andExpect(status().isOk()).andReturn();
		String jsonResponse = result.getResponse().getContentAsString();
		ObjectMapper objectMapper = new ObjectMapper();
		...
		mockMvc.perform(
				put(uri).contentType("application/json").content(objectMapper.writeValueAsString(orderToUpdate)))
				.andExpect(status().isOk());

		result = mockMvc.perform(get("/api/orders/" + orderId)).andExpect(status().isOk()).andReturn();
